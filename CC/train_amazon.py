...
from models_amazon import Discriminator
from models_amazon import CMSS
...
for target in range(4):
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        # srcs = [matrix[idx] for idx in range(4) if not idx == target]
        ...
        for source in range(3):
            ...
            for batch in range(max_len // opt.batchSize):
                ...
                for idx in range(batch * opt.batchSize, (batch + 1) * opt.batchSize):
                    ...
                    cur = source_reviews[source][temp]
                    cur = torch.tensor(cur, requires_grad=False).view(1, -1).to("cuda")
                    real_A.append(cur)
                    A_sentiments.append(source_sentiments[source][temp])
                ...
                for idx in range(batch * opt.batchSize, (batch + 1) * opt.batchSize):
                    ...
                    cur = target_reviews[temp]
                    cur = torch.tensor(cur, requires_grad=False).view(1, -1).to("cuda")
                    real_B.append(cur)
                    B_sentiments.append(target_sentiments[temp])
                ...
                for kkk in range(opt.batchSize):
                    ...
                    loss_GAN_A2B += temp
                    # print("sample_A loss: ", float(temp))
                ...
                loss_GAN_B2A = 0
                for kkk in range(opt.batchSize):
                    loss_GAN_B2A += criterion_GAN(pred_fake[kkk].view(1, -1), target_real[kkk].view(1))
                ...
                loss_G = loss_GAN_A2B + loss_GAN_B2A + loss_cycle_ABA + loss_cycle_BAB \
                         + sentiment_loss_A2B + sentiment_loss_B2A + loss_identity_A + loss_identity_B
                if batch % 10 > 2:
                    ...
                    print({'loss_G': float(loss_G), 'loss_G_GAN': float(loss_GAN_A2B + loss_GAN_B2A),
                           ...
                           'loss_sentiment': float(sentiment_loss_B2A + sentiment_loss_A2B),
                           'loss_S_A': float(S_A_loss), 'loss_S_B': float(S_B_loss)})
                ###################################
                else:
        ...
        torch.save(netG_A2B.state_dict(),
                   '/root/data/xiaoyang/PyTorch-CycleGAN-master/model/GAN2/' + str(target) + '/netG_A2B.pth')
        torch.save(netG_B2A.state_dict(),
                   '/root/data/xiaoyang/PyTorch-CycleGAN-master/model/GAN2/' + str(target) + '/netG_B2A.pth')
        torch.save(netD_A.state_dict(),
                   '/root/data/xiaoyang/PyTorch-CycleGAN-master/model/GAN2/' + str(target) + '/netD_A.pth')
        torch.save(netD_B.state_dict(),
                   '/root/data/xiaoyang/PyTorch-CycleGAN-master/model/GAN2/' + str(target) + '/netD_B.pth')
###################################
