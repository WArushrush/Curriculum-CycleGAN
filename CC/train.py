...
for target in range(5):
    ...
    # optimizer_emb_A = torch.optim.Adam(emb_A.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    # optimizer_emb_B = torch.optim.Adam(emb_B.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    # optimizer_E_A = torch.optim.Adam(encoder_A.parameters(), lr=opt.lr, betas=(0.5, 0.999))
    ...
    # lr_scheduler_emb_A = torch.optim.lr_scheduler.StepLR(optimizer_emb_A,
    #                                                    step_size=40, gamma=0.5)
    # lr_scheduler_emb_B = torch.optim.lr_scheduler.StepLR(optimizer_emb_B,
    #                                                    step_size=40, gamma=0.5)
    # lr_scheduler_E_A = torch.optim.lr_scheduler.StepLR(optimizer_E_A,
    #                                                  step_size=40, gamma=0.5)
    ...
    for epoch in range(opt.epoch, opt.n_epochs):
        srcs = [open(root + "/" + data_name[idx] + "_train_embedding_3.pkl", 'rb') for idx in range(5) if
                not idx == target]
        target_domain = open(root + "/" + data_name[target] + "_train_embedding_3.pkl", 'rb')
        source_reviews = []
        source_sentiments = []
        target_reviews = []
        target_sentiments = []
        ...
        for source in range(1):
            ...
            for batch in range(max_len // opt.batchSize):
                ...
                # encoder_output, encoder_hidden = encoder_A(original)
                ...
                # review_lengths = [review_lengths[int(idx)] for idx in idx_sort]
                # original = nn.utils.rnn.pack_padded_sequence(input=original, lengths=review_lengths, batch_first=True)
                # encoder_output, encoder_hidden = encoder_B(original)
                ...
                sentiment_loss_A2B = criterion_GAN(pred_sentiment_fake_B, A_sentiments)/10
        ...
        # lr_scheduler_emb_A.step()
        # lr_scheduler_emb_B.step()
        # lr_scheduler_E_A.step()
        ...
        # torch.save(emb_A.state_dict(),
        #            '/root/data/xiaoyang/PyTorch-CycleGAN-master/model/GAN/' + str(target) + '/emb_A.pth')
        # torch.save(emb_B.state_dict(),
        #            '/root/data/xiaoyang/PyTorch-CycleGAN-master/model/GAN/' + str(target) + '/emb_B.pth')
        # torch.save(encoder_A.state_dict(),
        #            '/root/data/xiaoyang/PyTorch-CycleGAN-master/model/GAN/' + str(target) + '/encoder_A.pth')
