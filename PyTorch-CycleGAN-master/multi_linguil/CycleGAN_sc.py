...
for lan in languages:
    for domain in domains:
        f = open(root+"/"+lan+"/"+domain+"_train_embedding_5.pkl",'rb')
        temp = pickle.load(f)
...
for lan in languages:
    for domain in domains:
        f = open(root+"/"+lan+"/"+domain+"_test_embedding_5.pkl",'rb')
        temp = pickle.load(f)
...
class SoftMax(nn.Module):
    def __init__(self, n_feature=1200,n_out=2):
        super(SoftMax, self).__init__()
        self.hidden1 = nn.Linear(n_feature, 600)
        # self.hidden2=nn.Linear(80,60)
        # self.hidden3=nn.Linear(60,40)
        # self.hidden4=nn.Linear(40,20)
        self.hidden2 = nn.Linear(600,300)
        self.hidden3 = nn.Linear(300,150)
        self.out = nn.Linear(150, n_out)
        self.model=nn.Sequential(self.hidden1, nn.LeakyReLU(), nn.Dropout(0.4),
                                 self.hidden2, nn.LeakyReLU(), nn.Dropout(0.3),
                                 self.hidden3, nn.LeakyReLU(), nn.Dropout(0.2),
...
for target in range(4):
    for domain in range(3):
        net_A = SoftMax(n_feature=1200, n_out=2).to("cuda")
        net_A = nn.DataParallel(net_A, device_ids=[0, 1])
        # net_A = torch.nn.parallel.DistributedDataParallel(net_A, device_ids=[0, 1])
        optimizer_A = torch.optim.Adam(net_A.parameters(), lr=0.0001, betas=(0.5, 0.999), weight_decay=0.0001)
        ...
        net_G.load_state_dict(
            torch.load('/root/data/xiaoyang/PyTorch-CycleGAN-master/model/GAN5/' + str(target) + '/' + str(domain) + '/netG_A2B.pth'))
        for epoch in range(epoches):
            ...
            for source in range(3):
                ...
                train_dataset = Data.TensorDataset(
                    torch.tensor(source_reviews[source], requires_grad=False).view(-1, 1200),
                    torch.tensor(source_sentiments[source], requires_grad=False))
                # train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
                loader = Data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=False, num_workers=1)
                print("loaded")
                for batch, (real_A, A_sentiments) in enumerate(loader):
                    ...
                    real_A = net_G(real_A)
                    A_sentiments = A_sentiments.to("cuda")
                    pred = net_A(real_A)
