...
class SoftMax(nn.Module):
    def __init__(self, n_feature=1024,n_out=2):
        ...
        self.model=nn.Sequential(self.hidden1, nn.LeakyReLU(),
                                 self.hidden2, nn.LeakyReLU(),
                                 self.hidden3, nn.LeakyReLU(),
...
for target in range(4):
    for domain in range(3):
        source_reviews = [matrix[target*3+domain]]
        target_reviews = test_matrix[target*3+domain]
        source_sentiments = [data_labels[target*3+domain]]
        ...
        for source in range(1):
            ...
            for epoch in range(epoches):
                ...
                train_dataset = Data.TensorDataset(torch.tensor(source_reviews[source], requires_grad=False).view(-1, 1024), torch.tensor(source_sentiments[source], requires_grad=False))
                train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
                loader = Data.DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=False, num_workers=2, sampler=train_sampler)
                ...
                print("target: ", target, "domain: ", domain, "epoch: ", epoch, "acc: ", acc/max_len, "res: ", res, "dev_loss: ",float(dev_avg_loss))
                lr_schedular_A.step()
print("result: ", res)